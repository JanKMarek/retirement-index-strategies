...
def run_backtest(
    ...
    cagr_benchmark, cagr_strategy, long_entries, ma_index_name, max_dd_benchmark, max_dd_strategy, res, sma_col = execute_strategy(
        backtest_end_date, backtest_start_date, ma_days, ma_index, trading_instrument, use_ma_filter, use_vix_filter,
        vix_threshold)

    display_with_plotly(backtest_end_date, backtest_start_date, cagr_benchmark, cagr_strategy, display_chart, ma_days,
                        ma_index_name, max_dd_benchmark, max_dd_strategy, res, sma_col, trading_instrument,
                        use_ma_filter, use_vix_filter, vix_threshold)

    return cagr_strategy, max_dd_strategy, strategy_parameters, long_entries


def display_with_plotly(backtest_end_date: str | Any, backtest_start_date: str, cagr_benchmark, cagr_strategy: str,
                        display_chart: bool, ma_days: int, ma_index_name: float | int | Any, max_dd_benchmark,
...
def execute_strategy(backtest_end_date: str | Any, backtest_start_date: str, ma_days: int, ma_index: str,
                     trading_instrument: str, use_ma_filter: bool, use_vix_filter: bool, vix_threshold: float) -> tuple[
    Any, str, str, float | int | Any, Any, float | int | Any, Any, Any]:
    ...
    res[f'Drawdown_{ma_index_name}'] = (res[f'{ma_index_name}_Rebased'] - res[f'Peak_{ma_index_name}']) / res[
        f'Peak_{ma_index_name}']
    max_dd_benchmark = res[f'Drawdown_{ma_index_name}'].min()
    max_dd_date_benchmark = res[f'Drawdown_{ma_index_name}'].idxmin()
    return cagr_benchmark, cagr_strategy, long_entries, ma_index_name, max_dd_benchmark, max_dd_strategy, res, sma_col


if __name__ == "__main__":
    # optimal config
    cagr, max_dd, params, long_entries = run_backtest(
